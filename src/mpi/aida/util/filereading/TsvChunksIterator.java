...
public class TsvChunksIterator implements Iterator<Pair<String[], List<String[]>>> {
  ...
  public TsvChunksIterator(
      File tsvFile, int[] groupingRange, boolean useRangeForData, 
      int progressAfterLines) 
  ...
  public Pair<String[], List<String[]>> next() {
    ...
    while (internalIterator_.hasNext()) {
      ...
      if (checkGroupingElementChanged(
          currentGroupingElement_, groupingElement)) {
        String[] key = Arrays.copyOf(currentGroupingElement_, 
            currentGroupingElement_.length);
        List<String[]> value = 
            new ArrayList<String[]>(currentChunkData_.size());
        for (String[] v : currentChunkData_) {
          value.add(v);
        }
        Pair<String[], List<String[]>> chunk = 
    ...
    return new Pair<String[], List<String[]>>(
        currentGroupingElement_, currentChunkData_);
  }
  ...
  private boolean checkGroupingElementChanged(
      String[] current,
      String[] next) {
